...
const agent = new https.Agent({
    cert: fs.readFileSync(path.join(__dirname, '../../../../static/certified/cert_dummy_lab_v2.crt')),
    key: fs.readFileSync(path.join(__dirname, '../../../../static/certified/cert_dummy_lab_key_v2.pem')),
})
...
export class DaviplataService {
    ...
    async auth() {
        ...
        try {
            const resp = await fetch(this.configService.get('URL_BASE_DAVIPLATA') + '/oauth2Provider/type1/v1/token', options);
            const responseData = await resp.json();
            return this.response(HttpStatus.OK, 'OK', 'Flujo de Autenticación Exitoso', responseData, false);
        } catch (error) {
    ...
    async buy(params: BuyDaviplataDto) {
        ...
        try {
            const resp = await fetch(this.configService.get('URL_BASE_DAVIPLATA') + '/daviplata/v1/compra', options);
            const responseData = await resp.json();
            ...
            return this.response(HttpStatus.OK, 'OK', 'Transacción compra sin error en el flujo', responseData, false);
        } catch (error) {
            return this.response(HttpStatus.INTERNAL_SERVER_ERROR, 'Error', 'Error en el servidor, intenta nuevamente', error, true);
        }
    }
    ...
    async otp(params: OtpDaviplataDto) {
        ...
        try {
            const resp = await fetch(this.configService.get('URL_BASE_DAVIPLATA') + '/otpSec/v1/read', options);
            const responseData = await resp.json();
            return this.response(HttpStatus.OK, 'OK', 'Transacción otp sin error en el flujo', responseData, false);
        } catch (error) {
            return this.response(HttpStatus.INTERNAL_SERVER_ERROR, 'Error', 'Error en el servidor, intenta nuevamente', error, true);
        }
    }
